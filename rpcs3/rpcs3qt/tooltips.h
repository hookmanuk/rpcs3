...
public:
	...
	const struct settings
	{
		...
		const QString vr_frame_rate                = tr("The game's frame rate while the headset is on (the vertical blanking follows it; the Vblank Rate setting applies again without a headset).\nUntil you choose a rate, each game runs the default its VR profile sets, one current hardware reaches (in a collection, each game has its own). Higher rates need a faster PC: if the game cannot keep up it can stutter or, in some games, drop objects for a frame. Unlimited follows the headset's refresh rate.\nRates above what the game works at are not offered, and a game never runs above its own maximum. For the smoothest result set the headset to the same refresh rate or a multiple of it.");
