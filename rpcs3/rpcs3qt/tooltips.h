...
public:
	...
	const struct settings
	{
		...
		const QString vr_enabled                   = tr("Renders the game in stereo and outputs it to an OpenXR headset.\nOnly games with a VR profile (bin/vr_profiles/<serial>.json) can use it, so this can only be ticked in such a game's own custom configuration. Requires a restart of the game.");
		...
		const QString vr_match_headset_rate        = tr("Runs the game's vertical blanking at the headset's refresh rate (e.g. 90 Hz) while the headset is on, so every headset frame gets a new game frame. The Vblank Rate setting is left as it is and applies again without a headset.\nOffered only for games whose VR profile confirms they keep normal speed at other rates. Needs a headset runtime that reports its refresh rate (XR_FB_display_refresh_rate, e.g. SteamVR); otherwise set Vblank Rate by hand.");
		const QString vr_match_headset_rate_unsupported = tr("Not supported in this game: its VR profile does not confirm that it keeps normal speed when the vertical blanking rate changes.");
